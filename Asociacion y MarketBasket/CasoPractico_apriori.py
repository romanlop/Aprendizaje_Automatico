...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 16 13:26:00 2019
...
def rules_from_conseq(freq_set, H, support_data, rules, min_confidence=0.5, verbose=True):
...
#####################################################################################
data = pd.read_csv("data/groceries_tuneado.csv",sep=";",header=None, keep_default_na=False) 
data_list = data.values.tolist()

#eliminamos elemenos nulos.
for l in data_list:
    while '' in l:
        l.remove('')


F, soporte = apriori(data_list, min_support = 0.15, verbose = True)

F, soporte = apriori(data_list, min_support = 0.05, verbose = False)

H = generate_rules(F, soporte, min_confidence = 0.25, verbose = True)